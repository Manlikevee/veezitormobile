...
const Setqr = ({ visible, onClose, modaldata,  }) => {
  const { axiosInstance, setVisitationdata  } = useContext(VeeContext);
  ...
  const handleLogout = async () => {
    ...
    setIsLoading(true);
    const payload = {
        "post_id": modaldata.mytrf,
        "tag_id": scanneddata
    }
    try {
      const url = `/verifyvisitor`
    ...
    >
      <ThemedView style={styles.modalOverlay} lightColor='' >
        ...
        <AntDesign name="closecircleo" size={24} color="#f2f2f2" />
          ...
          <ThemedText style={styles.modalText} lightColor='#000000e0' darkColor='#f2f2f2'>Scan the QR code using your mobile device. to assign Tag to {modaldata.first_name} {modaldata.last_name}.</ThemedText>
