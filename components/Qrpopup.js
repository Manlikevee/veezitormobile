  ...
  const { axiosInstance, fetchQrCode } = useContext(VeeContext);
    ...
    const handleSubmit = () => {
      ...
      .catch(error => {
        ...
        ToastAndroid.show(error?.response?.data?.detail || 'An Error Occured', ToastAndroid.SHORT);
        setIsLoading(false)
      });
    };
...
<ThemedView style={styles.jc} lightColor='transparent'>
   ...
   <AntDesign name="closecircleo" size={24} color={Colors[colorScheme ?? "light"].icon} />
   </TouchableOpacity>
  
</ThemedView>
<ThemedText style={styles.sectionbody} darkColor='#aaaaaa'>Fill The Below Details To Create A Tag</ThemedText>
...
<ThemedView  style={[
    ...
    <ThemedView>
    <MaterialIcons name="qr-code-scanner" size={20} color={Colors[colorScheme ?? "light"].icon} />
    </ThemedView>
    <ThemedView style={{flexGrow:1,  }}>
        <TextInput autoCapitalize="none"
               ...
               style={{ color:Colors[colorScheme ?? "light"].qrcolor}}/></ThemedView>
</ThemedView>
<TouchableOpacity
